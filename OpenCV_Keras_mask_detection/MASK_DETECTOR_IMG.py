...
while True : 
    ...
    image_path = str(input('Enter the path of image. Leave empty to exit : '))
    
    if image_path == '' :
        break
    
    test_img = cv2.imread(image_path)
    test_img = cv2.resize(test_img, (test_img.shape[1] * 3, test_img.shape[0] * 3))
    ...
    try : 
        ...
        if pred[0] > pred[1] : 
            ...
            cv2.putText(test_img, 'mask %.2f' %pred[0], (startx, starty + 25), cv2.FONT_HERSHEY_SIMPLEX, 
                        1, (255,255,100), 2)
        else : 
            cv2.rectangle(test_img, (startx,starty), (endx, endy), (255,0,0), 3)
            cv2.putText(test_img, 'no mask %.2f' %pred[1], (startx,starty + 25), cv2.FONT_HERSHEY_SIMPLEX,
                        1, (255,255,100), 2)
